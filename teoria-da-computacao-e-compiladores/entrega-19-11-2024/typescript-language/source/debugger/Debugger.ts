...
export function capture<T extends Nodes.Node>() {
	return function (_: any, __: any, descriptor: PropertyDescriptor) {
		...
		descriptor.value = function* (node: T): Generator<void | Value.BaseValue> {
			...
			__this.__debugger.lastEvaluation = null;
			...
			__this.__debugger.lastEvaluation = capture;
...
class Debugger {
	...
	public lastEvaluation: Value.BaseValue | null = null;
	...
	private __stepForward() {
		...
		this.__logger(`${this.stack[this.stack.length - 1]} -> ${this.lastEvaluation}`);
	}
